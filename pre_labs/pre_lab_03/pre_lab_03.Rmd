...
**Answer**amount=double  name=character  id=double
```{r}
glimpse(ppp_maryland_loans)
```
### Task 6: Change the "id" field data type
**Task** Run the following codeblock to change the data type of the "id" field from a number (dbl) to a character, and then glimpse the data, to see the change. Add a description of what this code does to your reference notebook.  Changes data type field of a code   
...
**Answer**:135073 and it is the same as forgiveness_amount. 
...
**Answer**:135073. They are both from the same records or the same thing. 
...
**Answer**1862 in October 2020. It changes everything to be on the first of every month. 	
```{r}
ppp_maryland_loans %>%
...
**Answer**Max=10000000	Min=6 This is suspicious because why is the minimum loan 6 dollars and the largest 10000000 dollars. Who needs that much or little money for loans?
```{r}
...
**Answer**The _ was not in between street address, name is written in all capital letters, and Arnold is spelled and written in many different ways.
...
**Answer**It made 1_id a double and it fixes certain titles to make sure it makes sense on the charts. 
...
### Task 7: Use rename()
**Task** Run the following codeblock to use the clean_names() function from janitor to standardize column names and then use rename() to change the "x1_id" column. Add a description of what this code does to your reference notebook.Made some of the titles change to read the columns better
...
Run `rlang::last_error()` to see where the error occurred. I think this means that something is out of place or it is not understanding what it's supposed to do. 
...
**Answer**1 business was duplicated twice and it was Stuart Jones LLC.
...
**Answer**5 rows. This code removes any duplicates your dataframe may have.
...
**Answer**This code makes sure that every ZIP code is the amount of numbers you want it to be
...
**Task** Run the following codeblock to use str_tot_title() to standarize capitalization in the "city" field. How many mispellings of Arnold remain after running this code? Answer below. Add a description of what this code does to your reference notebook. 2 misspellings of Arnold remained when the code was launched. This code tries to fix the misspellings of the city you're trying to look at.
...
**Answer**1 misspelling still remains after the code was lanuched. 
...
**Answer**There are no more misspellings of Arnold after this code was lanuched. This will elimnate any misspellings from the dataframe. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
