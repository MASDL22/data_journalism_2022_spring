...
**Answer**There are 195865 rows and 63 columns.
...
**Answer** It's bringing the names of the counties together by largest to smallest amounts of loans given.
...
**Answer**There is an empty gray square when the code ran.  
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**The dataframe was made into a bar chart, however, the names have little to no space between each other. 
...
**Answer**It reordered the counties based on smallest to largest amounts of loans.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**The bar chart is now sideways and a little easier to read.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**The gray background disappeared making the chart a little more easier to read.
...
**Answer**The bar chart is surrounded by a blue square which sorta made it a bit difficult to read some of the labels.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer**The labels have now changed from reorder(project_county_name,total_loans) and weight=total_loans to county and total loans.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**A dataframe of total loans from smallest loans and earliest date to largest loans and latest date. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**The chart is a line chart but there's a gap where no numbers are there making it appear as if no loans between July 2020 and January 2021.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**There are more dates in the chart which somewhat fixed the gap but they're all close together and unreadable.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**The dates are now placed in a 45 degree angle which allows us to read the dates better on the chart. 
```{r}
ppp_maryland_loans_by_date %>%
  ggplot() + 
  geom_line(aes(x=date_approved, y=total_loans)) + 
