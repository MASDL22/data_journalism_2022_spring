...
**Answer**A lot of charts based on things like sectors or divisions.
...
**Answer**There is the code that designs the entire page within the website.
...
**Answer**The code gave us the url within an html head and body. It tries to display the contents through html.
...
**Answer**It has the head and body of the html and says that there's a list of 2 without saying what the 2 is.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**It shows the code for sector has a stubhead and has an id of NAICSsectors-0-0.
...
**Answer**Some of the NAICS data came from 2002 up to 2017 on one output, another had the codes to different industries that NAICS has for them, and another doesn't look like a normal dataframe.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**Now the chart displays 6 different tibbles that all have different outputs within them.
...
**Answer**This dataframe mainly has dates within it and describes what year the NAICS data was taken.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer**THe second row has the sector and description of the industries on the NAICS.
...
**Answer**They have the sectors and descriptions of the industries within the dataframe. However, the source is seen as a description which it isn't.
...
**Answer**Now the names are all lowercased and the last row has been cut from the dataframe.
...
**Answer**The output shows the data being split between sectors and description.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**This link goes to NAICS 22 which is the code for utilities. It gives various data entries for different areas of the profession like employment and more. 
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer**It has the content type and the beginning header for the datatable on the website.
...
**Answer**The data is divided into different sections that make the datatable on the website readable and not out of order.
...
**Answer**The output looks a little bit off as there are footnotes are in the dataframe and the backdata isn't aligned with the data.
...
**Answer**It made data series lowercased and has the date to go along with it.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**This page has the results from the Mining, Quarrying, and Oil and Gas Extraction industry. It's similar to utilities but with its own results like it's average earnings increased from Oct 2021 to Jan 2022.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**The construction page had an increase of fatalities from 2017 to 2020 but their employment keeps increases as well.
...
**Answer**It listed out the industries in a certain order.
```{r}
...
**Answer**It listed all the industries into a sector.
```{r}
...
**Answer**The list mainly has the Mining, Quarrying, and Oil and Gas Extraction.
```{r}
...
# Make a for loop and run it
 #I need to make URL for sector, then scape url into hmtl and save it, then find specific table in html, then extract one row(second row) from table
ind<-"https://www.bls.gov/ces/naics/"
read_html() %>%
  html_element(xpath = '//*[@id="NAICSsectors"]')%>%
  print("Mining, Quarrying, and Oil and Gas Extraction")

for (sector in list_of_sectors) {
  print("Agriculture")
  print("Forestry, Fishing and Hunting")
  print("Mining, Quarrying, and Oil and Gas Extraction")
  print("Utilities")
  print("Construction")
  print("Manufacturing")
...
**Answer**It says the sector list is not found.
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**The output shows all the listed sectors from the database.
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer**It says that sector name can't be found.
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer**The output gives the list from 1-10.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer**It continues to have 20 rows with 2 sections.
...
**Answer**The list was numbered based on the dataframe.
...
**Answer**It gave the data series along with the results from Nov 21.
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer**The last row was removed, leaving only the sectors needed.
```{r}
# Define url of page we want to scrape
...
**Answer**Gives the dataframe from the url.
...
**Answer**It gives the sector and description along with the sector url and the xpath they came from.
...
**Answer**Now the dataframe only has 19 rows since Public Adminisration was removed.
...
**Answer*Each output had information about one industry.
...
**Answer**The list has all the URLs for the different industries along with an id to identify them. 
```{r}
...
**Answer**The output mainly had results from the employment and unemployment results within most of the outputs but it appears that they brought all the results into one dataframe at some points.
...
**Answer**All the data is on one row instead of two. 

```{r}

# For loop, iterating over each row in our naics industry dataframe
#for each row number in a range from 1-19 
for(row_number in 1:nrow(naics_industry)) {
...
**Answer**Now the names of the industries are in the codeblock but they're on the second page. 
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer**The database was able to keep all the information from the previous question but the only thing that changed was the lack of commas in the dataframe. 
...
**Answer**The outputs were now put into one chart instead of individual codeblocks. 
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer**The Agriculture, Forestry, Fishing and Hunting NACIS had instances where various sections like unemployment had increased but the fatalities decreased from 2017-2020.
...
**Answer**The Agriculture, Forestry, Fishing and Hunting has an NA when they refer to the November 2021 employees.
