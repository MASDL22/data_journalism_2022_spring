...
wv_populaton_county <- read_csv("data/wv_population_county.csv")
...
```{r}
wv_addresses <- read_csv("data/zip_2540_-loan_sample_csv.csv")
```
...
**A1.**126 E Burke St has the most loans with 6 loans to the address.BRIX27, LLC, DREW HOLDINGS LLC, HUB CO-OP LLC, 	
RONIN PROPERTIES LLC, and HUB CO-OP, LLC are located at that address.
...
**A2.**This isn't really suspicious because 126 E Burke St is a place that hosts various businesses for a fee and these businesses get a space based on the amount of people they will have. 
...
Berkeley<-read_csv("data/Berkeley_New.csv") 
...
**A3.**Martinsburg,WV has the most loans. Limited Liability Companies got the most loans in the city

```{r}
Berkeley%>%
  group_by(city_clean,business_type) %>%
  summarise(
    count=n()
  ) %>%
  arrange(desc(count))
...
**A4.**1. Starting an LLC might be easy to get in WV.   2. A lot of people have LLC's in the state.
...
**A5.**STEPPING STONES COTTAGES, LLC got the largest loan of 363400 dollars. It was approved on April 10, 2020. It retained 55 jobs. There really isn't a difference in the name and it only has one location. It didn't close down before or after it got the loan. 
```{r}
Berkeley%>%
  filter(business_type=="Limited  Liability Company(LLC)")%>%
group_by(name, business_type, amount, date_approved, current_approval_amount, jobs_retained,project_county_name)%>%
summarise(
    count=(amount)
  ) %>%
  arrange(desc(amount))
