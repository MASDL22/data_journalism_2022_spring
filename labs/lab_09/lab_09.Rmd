...
wv_april%>%
  ...
  scale_x_date(date_breaks = "1 day", date_labels = "%b-%d",limits = as.Date(c("2020-04-01", "2020-04-30"))) +
  theme(
    axis.text.x = element_text(angle = 45,  hjust=1)
  )+
 labs(
    title="West Virginia PPP Loans",
    x = "date\n",
    y = "\ntotal loans",
     caption = "source: SBA PPP loan database"
  )+
geom_point(
  mapping = (aes(x=date_approved, y=total_loans)),
  data = NULL,
  stat = "identity",
  position = "identity",
...
geom_text(aes())geom_text(aes(x=date_approved, y=total_loans, label=total_loans),hjust=1, vjust=0)
```
...
state_ppp_tot<-read_csv("data/state_ppp_all.csv")%>%
...
state_all<-state_ppp_tot%>%
inner_join(states, by = c("state"="NAME"))%>%
mutate(one_per_pop = total_ppp_loans/estimate*100000)%>%
 arrange(desc(one_per_pop))%>%
head(10)

state_all %>%
  ggplot() +
  geom_bar(aes(x=reorder(state,one_per_pop), weight=one_per_pop), fill = "blue") +
  coord_flip() + 
  labs(
    title=" Most PPP loans per 100,000 people by State",
    x = "states",
    y = "loans",
...
**A2.** South Dakota, North Dakota, Nebraska,Georgia,Iowa,Illinois, Louisiana,Florida,Mississippi,and Montana have the most loans per 100,000 people.
...
```
**A3.**  
It appears that most people had part of their loans forgiven. Some had a small percentage of their loans forgiven and others had all the loans forgiven which seems a bit normal for the most part. 
