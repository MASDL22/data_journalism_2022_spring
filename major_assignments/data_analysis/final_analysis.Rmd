...
#This question can't be answered because the second file of games in 2022 on the website is significantly larger than the first ones in 2020. It would be impossible to remove the extra games from the second file in order to match the first. 
  ...
  #Number 5:How do board games with longer playing times compare to shorter ones?  What board game category (fantasy, card game, deduction, etc.) was rated the highest and the lowest?
  
  
  
  
  
  playing_details<-game_details%>%
 group_by(primary,boardgamecategory,board_game_rank)%>%
    summarise(count= n())
  
  
playing_details%>%
  summarise(
    arrange (desc(boardgamecategory)))
