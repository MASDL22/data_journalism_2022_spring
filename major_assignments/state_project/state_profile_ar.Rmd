...
ar_counties%>%
mutate(NAMELSAD = toupper(str_remove_all(NAMELSAD,", Arkansas|County")))%>% 
 mutate(NAMELSAD = str_trim(NAMELSAD,side="both"))
 
 
ak_ppp%>%
  
  
  mutate(per_capita = ((july_2018-july_2017)/july_2017)*10000)
...
arrange(desc(count))%>%
  ...
  ar_naics_cbp<-ar_ppp%>%
  left_join(cbp, by = c("naics_code"="naics"))
