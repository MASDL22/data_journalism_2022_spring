...
ak_ppp%>%
  mutate(per_capita_100k= total_ppp_loans/population*100000)%>%
...
arrange(desc(count))%>%
  ...
  ar_naics_cbp<-ar_ppp%>%
  inner_join(cbp, by = "naics_code")%>%
select(naics_code,project_county_name,business_type)
  
    
ar_naics_cbp%>%
  remove(business_type=="Sole Proprietorship","Self-Employed Individuals")
summarize(
count = n()
    ) 
